...
class EngineECU(BaseECU):
    ...
    def __init__(self, logical_address: int = 0x0E00):
        super().__init__(logical_address)
        self.vin = b"WVWZZZENG00A00001"
    ...
    def handle_request(self, uds_payload: bytes) -> bytes:
        ...
        if sid == 0x10 and len(uds_payload) >= 2:
            ...
            if sub_function in session_map:
                ...
                logger.info(f"EngineECU (0x{self.logical_address:04X}) entered session: {self.state['session']}")
                # Positive response: SID + 0x40, sub-function, and dummy timing parameters
        ...
        # 0x11 ECU Reset
            ...
            if reset_type in (0x01, 0x02, 0x03): # Hard, KeyOffOn, Soft
                ...
                logger.info(f"EngineECU (0x{self.logical_address:04X}) reset requested: type {reset_type}")
                # Positive response: SID + 0x40, reset_type
        ...
        # 0x22 Read Data By Identifier
            ...
            if did == 0xF190: # VIN
                logger.info(f"EngineECU (0x{self.logical_address:04X}) reading VIN")
